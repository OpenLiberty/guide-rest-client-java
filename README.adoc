...
When you are done checking out your service, stop the Open Liberty server.  You must
stop your running Open Liberty server before you re-install it next time.
...
Add the `collectArtists` and `collectAlbums` helper methods. These methods will parse the JSON and
...
The methods that you created in the `Consumer` class could be written directly in the
`ArtistResource` class. However, if you are consuming a REST resource from a third
party service, you should separate your `GET` and `POST` requests from your data consumption.
...
If the tests pass, you see the following output:
