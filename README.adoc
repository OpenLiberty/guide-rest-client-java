...
* The [hotspot=21-22]`@JsonbProperty` annotation to map JSON keys to class instance members and vice versa.
...
* The [hotspot=19]`@JsonbCreator` and [hotspot=21-22]`@JsonbProperty` annotations to annotate a custom constructor.
...
The [hotspot=7-34 file=0]`Artist` and [hotspot=6-34 file=1]`Album` POJOs are ready for deserialization. 
...
The [hotspot=62-67 file=1]`JSON-B provider` has been added as a dependency in your [hotspot file=1]`pom.xml`
...
[source, java, linenums, role='code_column']
----
include::finish/src/test/java/it/io/openliberty/guides/consumingrest/ConsumingRestIT.java[tags=!comment]
----
...
For your test classes to have access to JSON-B, the [hotspot=62-67 file=1]`yasson` dependency has been added in your [hotspot file=1]`pom.xml`.
