...
The REST service that provides the artists and albums resources is already written for you. When the Liberty runtime is running, this service is accessible at the `\http://localhost:9080/artists` endpoint, which responds with the [hotspot]`artists.json` file.
...
When you are done checking out the service, exit dev mode by typing `q` in the command-line session where you ran the Liberty instance, and then press the `enter/return` key.
