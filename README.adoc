...
The [hotspot=Artist file=0]`Artist` object has two instance members [hotspot=name file=0]`name` and [hotspot=albumns file=0]`albums`, 
...
The [hotspot=JSON-B-provider file=1]`JSON-B provider` has been added as a dependency in your [hotspot file=1]`pom.xml`
...
[source, java, linenums, role='code_column hide_tags=comment']
----
include::finish/src/test/java/it/io/openliberty/guides/consumingrest/ConsumingRestTest.java[]
----
...
You can use the [hotspot=BeforeClass]`@BeforeClass` and `@AfterClass` annotations to perform any one time setup and teardown
tasks before and after all of your tests execute, as well as the [hotspot=Before]`@Before` and [hotspot=After]`@After` annotations
...
For your test classes to have access to JSON-B, the [hotspot=JSON-B-provider file=1]`yasson` dependency has been added in your `pom.xml`.
