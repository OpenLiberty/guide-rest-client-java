...
for you and is accessible at the following link when the server is running http://localhost:9080/artists[http://localhost:9080/artists^], which responds with the [hotspot]`artists.json`.
...
you will make updates to the code directly, and use development mode so that the Open Liberty server 
automatically reloads the configuration without restarting, so you can see the results.
...
When the server is running, you can find your REST service at http://localhost:9080/artists[http://localhost:9080/artists^]
...
The `jsonb-1.0` feature is included in the MicroProfile dependency that is specified in your [hotspot file=1]`pom.xml` file. 
...
You can now access the endpoints at http://localhost:9080/artists/total[http://localhost:9080/artists/total^] to see the total number of artists,
and at `\http://localhost:9080/artists/total/<artist>` to see a particular artist's total number of albums. 
...
Maven finds and executes all tests under `it/` and each test method must be marked with the [hotspot=test-1 hotspot=test-2 hotspot=test-3 hotspot=test-4]`@Test` annotation.

You can use the [hotspot=BeforeAll]`@BeforeAll` and `@AfterAll` annotations to perform any one time setup and teardown
tasks before and after all of your tests execute, as well as the [hotspot=BeforeEach]`@BeforeEach` and [hotspot=AfterEach]`@AfterEach` annotations
...
For your test classes to have access to JSON-B, the [hotspot=Yasson file=1]`yasson` dependency has been added in your [hotspot file=1]`pom.xml`.
...
When you are done checking out the service, exit development mode by typing `q` in the shell session where you ran the server, 
