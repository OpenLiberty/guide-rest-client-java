...
For the purpose of this guide, you are given two POJOs: [hotspot=13-46 file=0]`Artist` and [hotspot=13-48 file=1]`Album`. 
The [hotspot=34-40 file=0]`Artist` object has two instance members [hotspot=38 file=0]`name` and [hotspot=39 file=0]`albums`, 
which map to the artist name and the collection of the albums they have written. The [hotspot=33-41 file=1]`Album` object represents a 
single object within the album collection, and contains three instance members [hotspot=38 file=1]`title`, [hotspot=39 file=1]`artist`, and [hotspot=40 file=1]`ntracks`, which map to the album title, the artist who wrote the album, and the number of tracks the album contains.
...
The [hotspot=13-46 file=0]`Artist` and [hotspot=13-48 file=1]`Album` POJOs are ready for deserialization. 
Next, we'll learn to consume the JSON response from your REST service.

[role="code_command hotspot=13-85" subs="quotes"]
----
...
The JSON-B provider has been added as a dependency in your [hotspot=81-86 file=1]`pom.xml`

The [hotspot=30-40 file=0]`consumeWithJsonb` method in the [hotspot=27-85 file=0]`Consumer` class makes a [hotspot=33 file=0]`GET` request to the
running artist service and retrieves the JSON. Then binds the JSON into an `Artist`
array, use the [hotspot=34 file=0]`Artist[]` entity type in the [hotspot=34 file=0]`readEntity` call.
...
The [hotspot=44-54]`consumeWithJsonp` method makes in the [hotspot=27-84]`Consumer` class makes a [hotspot=47]`GET` request
to the running artist service and retrieves the JSON. This method then uses the
[hotspot=58-68]`collectArtists` and [hotspots=72-82]`collectAlbums` helper methods. These helper methods will
...
[role="code_command hotspot=30-83", subs="quotes"]
----
Create the `ArtistResource` class
`src/main/java/io/openliberty/guides/consumingrest/service/ArtistResource.java`
----

* The [hotspot=37-39]`getArtists` method provides the raw JSON data service that you accessed at the
beginning of this guide.

* The [hotspot=45-53]`getJsonString` method uses JSON-B to return the JSON as a string that will
be used later for testing.

* The [hotspot=60-70]`getTotalAlbums` method uses JSON-B to return the total number of albums present
in the JSON for a particular artist. The method returns -1 if this artist does not exist.

* The [hotspot=77-80]`getTotalArtists` method uses JSON-P to return the total number of artists
present in the JSON.

The methods that you wrote in the [hotspot=27-84]`Consumer` class could be written directly in the
[hotspot=30-83]`ArtistResource` class. However, if you are consuming a REST resource from a third
...
[role="code_command hotspot=30-132", subs="quotes"]
----
Create the `ConsumingRestTest` class 
`src/test/java/it/io/openliberty/guides/consumingrest/ConsumingRestTest.java` 
----

Maven finds and executes all tests under `it/` and each test method must be marked with the [hotspot=59]`@Test` annotation.

You can use the [hotspot=39]`@BeforeClass` and `@AfterClass` annotations to perform any one time setup and teardown
tasks before and after all of your tests execute, as well as the [hotspot=46]`@Before` and [hotspot=51]`@After` annotations
...
For your test classes to have access to JSON-B, the following dependency has been added in your [hotspot=88-93]`pom.xml`

The [hotspot=60-76]`testArtistDeserialization` test case checks that `Artist` instances created from
the REST data and those that are hardcoded perform the same.

The [hotspot=127-129]`assertResponse` helper method ensures that the response code you receive is valid (200).

=== Processing with JSON-B test

The [hotspot=81-93]`testJsonBAlbumCount` and [hotspot=98-106]`testJsonBAlbumCountForUnknownArtist` tests both use the `total/{artist}`
endpoint which invokes JSON-B.

The [hotspot=81-93]`testJsonBAlbumCount` test case checks that deserialization with JSON-B was done correctly
and that the correct number of albums is returned for each artist in the JSON.

The [hotspot=98-106]`testJsonBAlbumCountForUnknownArtist` test case is similar to [hotspot=81-93]`testJsonBAlbumCount`
but instead checks an artist that does not exist in the JSON and ensures that a
value of '-1' is returned.

=== Processing with JSON-P test

The [hotspot=111-120]`testJsonPArtistCount` test uses the `total` endpoint which invokes JSON-P. This test
