...
Which responds with the following [hotspot=1-32]`JSON`
...
For the purpose of this guide, you are given two POJOs: [hotspot=7-34 file=0]`Artist` and [hotspot=6-36 file=1]`Album`. 
The [hotspot=22-28 file=0]`Artist` object has two instance members [hotspot=23 file=0]`name` and [hotspot=24 file=0]`albums`, 
which map to the artist name and the collection of the albums they have written. The [hotspot=21-29 file=1]`Album` object represents a 
single object within the album collection, and contains three instance members [hotspot=22 file=1]`title`, [hotspot=23 file=1]`artist`, and [hotspot=24 file=1]`ntracks`, which map to the album title, the artist who wrote the album, and the number of tracks the album contains.
...
[role="code_command hotspot=15-63 file=2" subs="quotes"]
----
...
The [hotspot=16-26 file=2]`consumeWithJsonb` method in the [hotspot=15-63 file=2]`Consumer` class makes a [hotspot=19 file=2]`GET` request to the
running artist service and retrieves the JSON. Then binds the JSON into an `Artist`
array, use the [hotspot=20 file=2]`Artist[]` entity type in the [hotspot=20 file=2]`readEntity` call.
...
The [hotspot=28-38]`consumeWithJsonp` method makes in the [hotspot=15-63]`Consumer` class makes a [hotspot=31]`GET` request
to the running artist service and retrieves the JSON. This method then uses the
[hotspot=40-50]`collectArtists` and [hotspots=52-62]`collectAlbums` helper methods. These helper methods will
...
[role="code_command hotspot=18-65 file=1", subs="quotes"]
----
...
* The [hotspot45-55 file=1]`getTotalAlbums` method uses JSON-B to return the total number of albums present
...
Maven finds and executes all tests under `it/` and each test method must be marked with the [hotspot=43]`@Test` annotation.

You can use the [hotspot=26]`@BeforeClass` and `@AfterClass` annotations to perform any one time setup and teardown
tasks before and after all of your tests execute, as well as the [hotspot=33]`@Before` and [hotspot=38]`@After` annotations
...
For your test classes to have access to JSON-B, the following dependency has been added in your [hotspot=88-93 file=1]`pom.xml`

The [hotspot=44-60 file=0]`testArtistDeserialization` test case checks that `Artist` instances created from
the REST data and those that are hardcoded perform the same.

The [hotspot=103-105 file=0]`assertResponse` helper method ensures that the response code you receive is valid (200).
...
The [hotspot=63-75]`testJsonBAlbumCount` and [hotspot=78-86]`testJsonBAlbumCountForUnknownArtist` tests both use the `total/{artist}`
endpoint which invokes JSON-B.

The [hotspot=63-75]`testJsonBAlbumCount` test case checks that deserialization with JSON-B was done correctly
and that the correct number of albums is returned for each artist in the JSON.

The [hotspot=78-86]`testJsonBAlbumCountForUnknownArtist` test case is similar to [hotspot=81-93]`testJsonBAlbumCount`
...
The [hotspot=89-98]`testJsonPArtistCount` test uses the `total` endpoint which invokes JSON-P. This test
