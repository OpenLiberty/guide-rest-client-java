...
you will make updates to the code directly and automatically see the results using development mode.
...
Maven finds and executes all tests under `src/test/java/it/` and each test method must be marked with the [hotspot=test-1 hotspot=test-2 hotspot=test-3 hotspot=test-4]`@Test` annotation.
...
If you are satisfied with your application, run the Maven `package` goal to rebuild the WAR file in the `target` directory:
