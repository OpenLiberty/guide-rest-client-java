...
JSON-B will attempt to find a matching field with the same name. If a match cannot be found, 
...
with an exception. The Artist POJO does not require this annotation because all instance 
members match the JSON keys by name.
