...
for you and is accessible at the following link when the server is running `\http://localhost:9080/artists`, which responds with the [hotspot]`artists.json`.
...
Run the following curl command to retrieve a particular the artist **bar**'s total number of albums:
```
