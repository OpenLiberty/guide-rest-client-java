...
[source, json, role="no_copy", role='code_column']
----
...
The JSON-B provider has been added as a dependency in your [hotspot=81-86]`pom.xml`

The [hotspot=30-40]`consumeWithJsonb` method in the [hotspot=27-85]`Consumer` class makes a [hotspot=33]`GET` request to the
running artist service and retrieves the JSON. Then binds the JSON into an `Artist`
array, use the [hotspot=34]`Artist[]` entity type in the [hotspot=34]`readEntity` call.
